...
**Versión del Documento:** V5

> Hicimos una revisión cruzada del documento completo (no solo la parte de disponibilidad de horarios) y encontramos varias cosas que convenía cerrar antes de pasar a diseño: la forma en que hablábamos de "solapamiento" de citas no correspondía a la regla que realmente implementamos, RF-09 tenía una ambigüedad real (paciente + médico no identifica una sola cita), mezclábamos "eliminar" con "dar de baja" a un médico sin haber decidido cuál de las dos es, y teníamos valores propuestos (edad, rendimiento, cobertura) escritos como si ya fueran definitivos. Todo eso quedó ajustado en esta versión; el detalle está en el Apéndice 4.2.
...
- **Lo que HACE el sistema:** Permite la administración básica operativa de una clínica: registro y actualización de pacientes, registro y baja lógica de médicos, agendamiento y cancelación de citas médicas validando que el médico esté disponible en la fecha y hora exacta solicitada (no manejamos duración de cita en esta fase, así que la validación evita que dos citas queden en el mismo horario exacto, no solapamientos parciales), registro de diagnósticos y tratamientos por consulta, y visualización cronológica del historial clínico.
...
2. **Gestión de Médicos:** Registrar personal médico, listar médicos registrados y dar de baja a un médico de forma lógica (queda marcado como INACTIVO, no se elimina físicamente, verificando además la inexistencia de citas activas agendadas). Optamos por la baja lógica porque un médico puede tener citas antiguas e historias clínicas asociadas, y borrarlo físicamente rompería esa trazabilidad.
...
- **Dominio / Reglas de Negocio:** No se permite dar de baja a un médico que tenga citas en estado AGENDADA.
...
- **RF-04.1:** Dado un ID único, nombre, edad dentro del rango laboral definido por el equipo (ver Apéndice 4.4, Ítem 1), teléfono válido y especialidad no vacía, cuando se confirma el registro, entonces el sistema lo guarda satisfactoriamente.
...
**[RF-06] Dar de Baja a un Médico (Prioridad: Could Have (Baja) / HU-06)**

*Descripción:* Permite dar de baja lógica a un médico evaluando integridad referencial y de agenda. El médico no se elimina físicamente de la base de datos: queda marcado como INACTIVO para conservar la trazabilidad de sus citas e historias clínicas asociadas, y deja de estar disponible para nuevas citas.

- **RF-06.1:** Dado un ID de médico existente sin citas futuras en estado AGENDADA, cuando se confirma la baja, entonces el sistema cambia su estado a INACTIVO.
- **RF-06.2:** Dado un ID de médico existente con citas futuras AGENDADA, cuando se intenta dar de baja, entonces el sistema rechaza la operación e instruye reasignar o cancelar dichas citas.
- **RF-06.3:** Dado un ID no existente, cuando se solicita dar de baja, entonces reporta error.
...
*Descripción:* Permite al médico documentar la atención brindada a un paciente, asociada a una cita específica. Identificamos la atención por el **ID de la cita**: un mismo paciente y médico pueden tener varias citas a lo largo del tiempo, así que "paciente + médico" no basta para señalar cuál de ellas se está atendiendo.

- **RF-09.1:** Dado un ID de Cita existente en estado AGENDADA, junto con diagnóstico y tratamiento, cuando se guarda, entonces el sistema anexa una nueva entrada a la Historia Clínica del paciente asociado a esa cita y la cita pasa a estado ATENDIDA.
- **RF-09.2:** Dado un ID de Cita inexistente, en un estado distinto a AGENDADA, o con diagnóstico/tratamiento vacíos, cuando se intenta guardar, entonces el sistema arroja un error de validación específico para cada caso.
...
Estos son metas de referencia para el MVP; todavía no las hemos validado con pruebas de carga reales sobre el stack elegido (ver Apéndice 4.4, Ítem 7):

- **Tiempo de Respuesta:** Las búsquedas, registros y validaciones de disponibilidad de horario en memoria/BD deberían resolverse en menos de 200 ms por transacción.
- **Capacidad:** El sistema en esta fase MVP debería soportar la gestión simultánea de al menos 1,000 pacientes, 100 médicos y 5,000 citas sin degradación notable.
...
- **Fiabilidad:** Las reglas de negocio (p. ej. la validación de disponibilidad de horario del médico y la baja lógica de médicos) deben ser probadas con una cobertura de líneas de código superior al 85%, medida con una herramienta de cobertura para Java (p. ej. JaCoCo).
- **Usabilidad:** Interfaz web intuitiva, clara y con mensajes descriptivos de éxito/error.

### 3.6. Otros Requisitos

- **Manejo de Errores y Logs:** Ninguna excepción no controlada (p. ej. `NullPointerException`, `IndexOutOfBoundsException`) debe detener abruptamente la ejecución del backend. Al usuario le mostramos siempre un mensaje genérico y amigable (p. ej. "No fue posible completar la operación. Intente nuevamente."), sin exponer detalles técnicos. El detalle técnico (tipo de excepción, stack trace, consulta SQL fallida, etc.) se registra únicamente en los logs del backend, nunca en el frontend.
...
**V4 → V5** (después de una revisión cruzada de todo el documento, no solo de la parte de disponibilidad de horarios):

| # | Sección | Qué encontramos | Qué decidimos |
|---|---|---|---|
| 16 | 1.2, 3.5 | Hablábamos de "validación estricta de solapamiento de horarios", pero la regla que realmente implementamos (RF-07.4) solo compara igualdad exacta de fecha/hora, no detecta solapamientos parciales (porque no manejamos duración de cita). | Ajustamos la redacción a "validación de disponibilidad del médico para evitar dos citas en la misma fecha y hora exacta", que es lo que el sistema realmente hace. |
| 17 | RF-09, CU-09 | Identificábamos la atención por "paciente + médico", pero un mismo paciente puede tener varias citas con el mismo médico en fechas distintas, así que esa combinación no identifica una sola cita. | Volvimos a usar el ID de Cita como identificador único de la atención, dejando claro por qué en la descripción del requisito. |
| 18 | 2.2, 2.4, RF-06, CU-06 | Usábamos "eliminar" y "dar de baja" como si fueran lo mismo, pero RF-06 decía literalmente "el sistema remueve el registro" (eliminación física), lo cual pondría en riesgo la trazabilidad de citas e historias clínicas de ese médico. | Decidimos que la baja de un médico es lógica: el registro se conserva y pasa a estado INACTIVO. Ajustamos la terminología en todo el documento. |
| 19 | RF-04.1, CU-04 | El requisito formal incluía la frase "todavía no lo confirmamos", lo cual no debería estar dentro de un requisito que se supone definitivo. | Dejamos el requisito en términos definitivos ("rango definido por el equipo") y movimos el valor propuesto (18–99) únicamente al Apéndice 4.4. |
| 20 | 3.3 | Los números de rendimiento (200 ms, 1.000/100/5.000 registros) estaban escritos como si ya fueran metas validadas, sin que hubiéramos hecho pruebas de carga. | Los dejamos como metas de referencia para el MVP y agregamos la validación con pruebas de carga como cuestión abierta. |
| 21 | 3.5 | El 85% de cobertura no especificaba qué tipo de cobertura (líneas, ramas, instrucciones) ni con qué herramienta se mediría. | Precisamos que es cobertura de líneas de código, medida con una herramienta para Java (p. ej. JaCoCo). |
| 22 | 3.6 | Decíamos que "todo error" se muestra al usuario, lo cual podría exponer información técnica sensible (excepciones, detalles de BD). | Separamos el mensaje genérico para el usuario del detalle técnico, que ahora va únicamente a los logs del backend. |
...
| **Descripción** | **Flujo básico:** 1) Se selecciona "Registrar médico". 2) El sistema solicita ID, nombre, edad, teléfono y especialidad. 3) Se ingresan los datos. 4) El sistema valida el rango de edad definido por el equipo (Apéndice 4.4, Ítem 1) y que la especialidad no esté vacía. 5) Se guarda y confirma. **Flujos alternos:** A1 (edad fuera de rango) → se solicita de nuevo; A2 (especialidad vacía) → se exige valor. |
...
| Campo | Detalle |
|---|---|
| **Caso de Uso** | Dar de Baja a un Médico |
| **Identificador** | CU-06 |
| **Actores** | Administrador del Sistema |
| **Tipo** | Opcional (Could Have - Baja) |
| **Referencias** | RF-06 / HU-06 |
| **Precondición** | Debe existir el médico y el usuario conoce su ID. |
| **Postcondición** | El médico queda marcado como INACTIVO (baja lógica) si no tiene citas AGENDADA; en caso contrario, no se modifica ningún dato. El registro del médico se conserva para mantener la trazabilidad de sus citas e historias clínicas. |
| **Descripción** | **Flujo básico:** 1) Se ingresa el ID del médico. 2) El sistema verifica que exista y no tenga citas AGENDADA. 3) Se marca como INACTIVO y confirma. **Flujos alternos:** A1 (citas activas) → se rechaza, se solicita reasignar/cancelar; A2 (ID inexistente) → error, sin cambios. |
| **Resumen** | Permite dar de baja lógica a un médico que ya no labora en la clínica, si no tiene compromisos de agenda activos, sin perder su historial. |
...
| **Precondición** | Debe existir una cita en estado AGENDADA. |
| **Postcondición** | Se anexa una nueva entrada a la historia clínica del paciente asociado a la cita, y la cita pasa a ATENDIDA. |
| **Descripción** | **Flujo básico:** 1) El médico ingresa el ID de la cita, el diagnóstico y el tratamiento. 2) El sistema valida que la cita exista y esté en estado AGENDADA. 3) Se guarda el registro en la historia clínica del paciente asociado a la cita y se actualiza la cita a ATENDIDA. 4) Se confirma. **Flujo alterno:** A1 (cita inexistente, en otro estado, o datos vacíos) → error, no se guarda. |
...
7. **Metas de rendimiento (3.3):** los 200 ms por transacción y la capacidad de 1.000 pacientes / 100 médicos / 5.000 citas son metas de referencia, no resultados medidos. Validarlas con pruebas de carga reales sobre el stack elegido, y ajustarlas si no son realistas.
